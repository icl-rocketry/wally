...
int PIN_LORA_RESET = A1;   // LORA RESET
int PIN_LORA_DIO_1 = A3;   // LORA DIO_1
int PIN_LORA_BUSY = A2;    // LORA SPI BUSY
int PIN_LORA_NSS = A0;     // LORA SPI CS
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr) {
    ...
    if (isMaster == true) {
        if (BufferSize > 0) {
            ...
            } else if (strncmp((const char *)RcvBuffer, (const char *)PingMsg, 4) == 0) {  // A master already exists then become a slave
